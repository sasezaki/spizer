...
$queue = new Kumo_Request_MessageQueue($queueConfig['queueAdapter'], $queueConfig['queueOptions']);
...
for ($i = 0; $i < $queue->count(); $i++) {
    $messages = $queue->receive(1, 1); //timeout 1?
    $message = $messages->current();

    var_dump($i, "--", $message->getBody());die();

    //if (($end = $starttime + $timeout) < time()) break;
}
*/

// Set up engine
$engine = new Kumo_Engine((array) $config->engine);
...
// Go!
$engine->doProgressBar(true);
$engine->setProgressBarAdapter($adapter);
